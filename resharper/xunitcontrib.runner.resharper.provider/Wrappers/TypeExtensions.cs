...
namespace XunitContrib.Runner.ReSharper.UnitTestProvider
{
	internal static class TypeExtensions
	{
		...
		{
			return type.FullName == c.GetClrName().FullName ||
			       c.GetAllSuperTypes().Any(superType => type.FullName == c.GetClrName().FullName);
		}

		internal static bool IsAssignableFrom(this Type type, IDeclaredType c)
		{
			return type.FullName == c.GetClrName().FullName ||
			       c.GetAllSuperTypes().Any(superType => type.FullName == superType.GetClrName().FullName);
		}
		...
		internal static bool IsAssignableFrom(this Type type, IMetadataTypeInfo c)
		{
			...
			return type.FullName == c.FullyQualifiedName ||
			       (c.Base != null && type.IsAssignableFrom(c.Base.Type));
		}
	}
}