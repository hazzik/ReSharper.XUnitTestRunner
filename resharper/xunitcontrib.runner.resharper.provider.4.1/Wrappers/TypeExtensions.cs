...
namespace XunitContrib.Runner.ReSharper.UnitTestProvider
{
    internal static class TypeExtensions
    {
        ...
        {
            return type.FullName == c.CLRName || MiscUtil.GetAllSuperTypes(c).Any(superType => type.FullName == c.CLRName);
        }

        internal static bool IsAssignableFrom(this Type type, IDeclaredType c)
        {
            return type.FullName == c.GetCLRName() || MiscUtil.GetAllSuperTypes(c).Any(superType => type.FullName == superType.GetCLRName());
        }
